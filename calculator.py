...
class Calculator(QWidget):
    def __init__(self):
        ...
        self.b_decimal.clicked.connect(self._decimal)
    ...
    def _operation(self,op):
        if not self.input.text():
            return
        self.num_1 = decimal.Decimal(self.input.text())
        self.op = op
        self.input.setText("")
    ...
    def _result(self):
        if not self.input.text():
            return
        self.num_2 = decimal.Decimal(self.input.text())
        if self.num_1 == None:
            return
        if self.op == "+":
        ...
        if self.op == "/":
            if self.num_2 == 0:
                self.error_message = self.input.setText("Деление на ноль невозможно")
            else:
                self.input.setText(str(self.num_1 / self.num_2))
        else:
            return
